    ...
    def _canon(self, v: Any) -> Any:
        ...
        if isinstance(v, np.ndarray):
            try:
                ...
                return ("nd", shp, dtype, self._canon_float(mean), self._canon_float(var))
            except Exception:
        ...
        if isinstance(v, Tensor):
            try:
                ...
                dtype = str(v.dtype)
                with torch.no_grad():
                    if hasattr(torch, "nanmean") and hasattr(torch, "nanvar"):
                        mean = float(torch.nanmean(t).item())
                        var = float(torch.nanvar(t, unbiased=False).item())
                    else:
                ...
                return ("t", shp, dtype, self._canon_float(mean), self._canon_float(var))
            except Exception:
