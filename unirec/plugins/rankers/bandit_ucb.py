...
class UCBSequentialSlate(Ranker):
    def setup(self, resources):
        ...
        self.prop_temp = float(
            self.params.get("propensity_temperature", 0.5)
        )  # for soft propensities
        ...
        if emb_res is not None:
            ...
            self.item_emb = (
                np.load(emb_res).astype(np.float32)
        ...
        for t in range(min(self.K, len(pool))):
            ...
            for c in pool:
                if any(ci.item_id == c.item_id for ci in chosen):
                    continue
                mu = state.context.get("precomputed_ctr", {}).get(
            ...
            logits = np.array([s for (_, _, _, s) in scores], dtype=np.float32) / max(
                self.prop_temp, 1e-6
            )
